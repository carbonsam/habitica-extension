...
const USER_EXPERIENCE_BAR_ELEMENT = document.getElementById(
  "hero_experience_bar"
);
const USER_EXPERIENCE_VALUE_ELEMENT = document.getElementById(
...
function loadSettings() {
  let id = chrome.storage.local.get("user_id", function(items) {});
  let key = chrome.storage.local.get("user_key", function(items) {});
...
function updatePopup(userData) {
  USER_IMG_ELEMENT.src =
    "https://habitica.com/export/avatar-" + USER_ID + ".png";

  USER_HEALTH_BAR_ELEMENT.style =
    "width: " +
    userData.data.stats.hp / userData.data.stats.maxHealth * 100 +
    "%";
  USER_HEALTH_VALUE_ELEMENT.innerHTML =
    Math.round(userData.data.stats.hp) + " / " + userData.data.stats.maxHealth;

  USER_EXPERIENCE_BAR_ELEMENT.style =
    "width: " +
    userData.data.stats.exp / userData.data.stats.toNextLevel * 100 +
    "%";
  USER_EXPERIENCE_VALUE_ELEMENT.innerHTML =
    userData.data.stats.exp + " / " + userData.data.stats.toNextLevel;
